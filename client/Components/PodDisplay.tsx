...
const PodDisplay: FC = () => {
  ...
  // upon component load, fetch custom metrics data to render graphs for pods and push element into custom graphs state
  ...
  return (
    <div>
      ...
      <div id="metric-graphs">
        <LineGraph
          label={`CPU Usage: ${pod}`}
          query={`/api/pod/cpu?pod=${pod}`}
          backgroundColor="rgba(54, 162, 235, 0.2)"
          borderColor="rgba(54, 162, 235, 1)"
          yAxisType="percent"
        />
        <LineGraph
          label={`Memory Usage: ${pod}`}
          query={`/api/pod/mem?pod=${pod}`}
