...
const podController: PodController = {
  ...
  getCpuUsage: async (req, res, next) => {
    ...
    try {
      ...
      return next({
        ...
        status: 500,
        message: {err: 'Error occured while retrieving pod cpu usage data'},
      });
    }
  },
  ...
  getMemUsage: async (req, res, next) => {
    const { pod } = req.query;
     try {
      const data = await axios.get(`http://localhost:9090/api/v1/query_range?query=container_memory_usage_bytes{pod='${pod}'}&start=${start}&end=${end}&step=10m`)
      ...
      return next({
        ...
        status: 500,
        message: {err: 'Error occured while retrieving pod memory usage data'},
      });
     }
  },
